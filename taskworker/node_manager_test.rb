...
class NodeManagerTest < Test::Unit::TestCase
  ...
  def test_get_puppet_additional_facts
    facts = get_puppet_additional_facts(:region => 'uswest1',
                                        :service => 'nextdoor.com,taskworker',
                                        :release_version => 'abcd')
    assert(facts == 'text:shard=uswest1 nsp="nextdoor.com=abcd,taskworker=abcd"')

    facts = get_puppet_additional_facts(:region => 'uswest1',
                                        :service => 'nextdoor.com,hello,taskworker',
                                        :release_version => 'abcd')
    assert(facts == 'text:shard=uswest1 nsp="nextdoor.com=abcd,hello=abcd,taskworker=abcd"')

    facts = get_puppet_additional_facts(:region => 'uswest1',
                                        :service => 'taskworker',
                                        :release_version => 'abcd')
    assert(facts == 'text:shard=uswest1 nsp="taskworker=abcd"')
  end
