...
class NodeManagerTest < Test::Unit::TestCase
  ...
  def test_get_puppet_facts
    ...
    assert(facts == 'array:["text:app_group=us2","text:nsp=nextdoor.com=abcd taskworker=abcd"]')

    facts = get_puppet_facts(:region => 'uswest1',
                             :app_group => 'us3',
                             :service => 'nextdoor.com,hello,taskworker',
                             :release_version => 'abcd')
    assert(facts == 'array:["text:app_group=us3","text:nsp=nextdoor.com=abcd hello=abcd taskworker=abcd"]')

    facts = get_puppet_facts(:region => 'uswest1',
                             :app_group => 'us1',
                             :service => 'taskworker',
                             :release_version => 'abcd')
    assert(facts == 'array:["text:app_group=us1","text:nsp=taskworker=abcd"]')
  end
