...
# Constructs puppet additional facts string for server array input.
#
# * *Args*    :
#   - +region+ -> string for AWS regions, e.g., uswest2
#   - +service+ -> comma separated string for packages to install. The last package
...
#
# * *Returns* :
#   - string for puppet additional facts, e.g.,
#     text:shard=uswest1 nsp="dep1=release1,dep2=release1,service=release1"
#
def get_puppet_additional_facts(args)
  region = args[:region]
  service = args[:service]
  release_version = args[:release_version]
  packages = service.split(',')
  puppet_facts = "text:shard=#{region} nsp=\""
...
def create_server_array(args)
  ...
  $log.info("SUCCESS. Created server array #{server_array_name}")
  puppet_facts = get_puppet_additional_facts(:region => :region,
                                             :service => service,
                                             :release_version => release_version)
  new_server_array.show.next_instance.show.inputs.multi_update('inputs' => {
            'PUPPET_ADDITIONAL_FACTS' => puppet_facts})
