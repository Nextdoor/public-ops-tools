...
create_apt_sources() {
  ...
  cat <<EOF >> /etc/apt/sources.list.d/jenkins.sources.list
...
EOF
  ...
  # Now, install the Nextdoor private repos
  cat <<EOF >> /etc/apt/sources.list.d/s3.sources.list
...
install_packages() {
  ...
  DEBIAN_FRONTEND=noninteractive apt-get -y --force-yes -q install $DEFAULT_PACKAGES $RUBY_PACKAGES
...
install_ruby() {
  ...
  rvm use --install --create 1.8.7
  rvm use --install --create 1.9.3
}
