...
def get_root_logger(level, syslog=None):
    level_string = 'logging.%s' % level.upper()
    level_constant = utils.str_to_class(level_string)
    return utils.setup_logger(level=level_constant, syslog=syslog)


def main():
    get_root_logger(options.level)
    api = operators.RightScaleOperator(token=options.token, api=options.url)

    results = api.run_script_on_server_array(
        name=options.server,
        script=options.script,
        noop=options.noop,
        wait=int(options.wait))

    print "Job Completed. Results below."
