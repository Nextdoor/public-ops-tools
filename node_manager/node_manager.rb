...
def get_puppet_facts(old_inputs, region, env, release_version)
  ...
  for fact in old_facts.split(',')
    if fact.start_with? 'nsp='
      if fact.count('=') > 1
        abort('This script does not support NSP defs with version pinning')
      end

      # Append version number to the NSPs
      nsp = fact.split('=').last.split()
      nsp = nsp.map { |nsp| "#{nsp}=#{release_version}" }
      fact = 'nsp=' + nsp.join(' ')
    end
    new_facts.push(fact)
  end
...
def clone_server_array(dryrun, right_client, tmpl_server_array,
  ...
  # Wait for the new array and instances to register in RightScale
  sleep 60
